dati_pois<-read.csv("prob_pois.csv", header = T, sep = ",")
dati_pois<-dati_pois[-which(is.na(dati_pois$X_prob_poi)),]
...
dati_pois$Data<-as.Date(dati_pois$Data, format="%d/%m/%Y")
...
dati_pois$H_team<-as.character(dati_pois$H_team)
dati_pois$H_team[which(dati_pois$H_team=="ACF Fiorentina")]<-"Fiorentina"
dati_pois$H_team[which(dati_pois$H_team=="Benevento Calcio")]<-"Benevento"
dati_pois$H_team[which(dati_pois$H_team=="Bologna FC")]<-"Bologna"
dati_pois$H_team[which(dati_pois$H_team=="Brescia Calcio")]<-"Brescia"
dati_pois$H_team[which(dati_pois$H_team=="Cagliari Calcio")]<-"Cagliari"
dati_pois$H_team[which(dati_pois$H_team=="Carpi FC")]<-"Carpi"
dati_pois$H_team[which(dati_pois$H_team=="Chievo Verona")]<-"Chievo"
dati_pois$H_team[which(dati_pois$H_team=="Empoli FC")]<-"Empoli"
dati_pois$H_team[which(dati_pois$H_team=="FC Crotone")]<-"Crotone"
dati_pois$H_team[which(dati_pois$H_team=="Frosinone Calcio")]<-"Frosinone"
dati_pois$H_team[which(dati_pois$H_team=="Genoa CFC")]<-"Genoa"
dati_pois$H_team[which(dati_pois$H_team=="Hellas Verona")]<-"Verona"
dati_pois$H_team[which(dati_pois$H_team=="Lazio Roma")]<-"Lazio"
dati_pois$H_team[which(dati_pois$H_team=="Parma Calcio 1913")]<-"Parma"
dati_pois$H_team[which(dati_pois$H_team=="Pescara Calcio")]<-"Pescara"
dati_pois$H_team[which(dati_pois$H_team=="Sassuolo Calcio")]<-"Sassuolo"
dati_pois$H_team[which(dati_pois$H_team=="SPAL 2013 Ferrara")]<-"Spal"
dati_pois$H_team[which(dati_pois$H_team=="SSC Napoli")]<-"Napoli"
dati_pois$H_team[which(dati_pois$H_team=="Torino FC")]<-"Torino"
dati_pois$H_team[which(dati_pois$H_team=="Udinese Calcio")]<-"Udinese"
dati_pois$H_team[which(dati_pois$H_team=="US Lecce")]<-"Lecce"
dati_pois$H_team[which(dati_pois$H_team=="US Palermo")]<-"Palermo"
dati_pois$H_team<-as.factor(dati_pois$H_team)
levels(dataset_finale$home_team)
levels(dati_pois$H_team)
dati_pois$H_team[1]==dataset_finale$home_team
b<-c()
for (i in 1:nrow(dataset_finale)){
  for (j in 1:nrow(dati_pois))
    if (dataset_finale$home_team[i]==dati_pois$H_team[j] & dataset_finale$date[i]==dati_pois$Data[j]) {c<-c(i,which(dataset_finale$home_team[i]==dati_pois$H_team & dataset_finale$date[i]==dati_pois$Data));b<-rbind(b,c)}
  print(i)
}
new_pois<-as.data.frame(b)
...
new_pois$poi1<-dati_pois$X1_prob_poi[new_pois$V2]
new_pois$poi2<-dati_pois$X2_prob_poi[new_pois$V2]
new_pois$poix<-dati_pois$X_prob_poi[new_pois$V2]

#soglia 5% pois
new_pois$scomm1<-new_pois$scomm2<-new_pois$scommx<-NA
new_pois$scomm1<-ifelse(new_pois$poi1>(new_pois$p1max+0.05),1,0)
new_pois$scomm2<-ifelse(new_pois$poi2>(new_pois$p2max+0.05),1,0)
new_pois$scommx<-ifelse(new_pois$poix>(new_pois$pxmax+0.05),1,0)
sum(new_pois$scomm1)
sum(new_pois$scomm2)
sum(new_pois$scommx)
...
(gain_stategy<-cumsum(mat$gain)[j])#-0396
-10396/1464 #perdita 7.10 a schedina
...
new_pois$scomm1_diff<-new_pois$scomm2_diff<-new_pois$scommx_diff<-NA
new_pois$scomm1_diff<-ifelse(new_pois$poi1>(new_pois$p1max+0.03-diff[1]),1,0)
new_pois$scomm2_diff<-ifelse(new_pois$poi2>(new_pois$p2max+0.03-diff[2]),1,0)
new_pois$scommx_diff<-ifelse(new_pois$poix>(new_pois$pxmax+0.03-diff[3]),1,0)
sum(new_pois$scomm1_diff)
sum(new_pois$scomm2_diff)
sum(new_pois$scommx_diff)
dsk_diff<-new_pois[-which(new_pois$scomm1_diff==0&new_pois$scomm2_diff==0&new_pois$scommx_diff==0),]
...
(gain_stategy<-cumsum(mat_diff$gain)[j])#-9484
-9484/1500 #perdita 6.32 a schedina
